...
def _generate_mt_coefficient_mapping(
    ...
    if delays and len(delays) >= 2:
        ...
        mapping.update({f"c{i}": f"Linear term for delay τ{i + 1}" for i in range(base_terms)})
...
def convert_to_dataframe(
    ...
    if long_format:
        ...
        for window_idx in range(n_windows):
            for channel_idx in range(n_channels):
                ...
                if time_column and "window_info" in dda_output:
                    row["start_index"] = dda_output["window_info"]["start_indices"][window_idx]
                ...
                if multi_channel:
                    ...
                    for coeff_idx in range(coefficients.shape[1]):
                        row[f"c{coeff_idx}"] = coefficients[window_idx, coeff_idx, channel_idx]
        ...
        if multi_channel:
            ...
            for channel_idx in range(n_channels):
                channel_name = (
                    metadata.get("channel_names", [f"ch{i}" for i in range(n_channels)])[
                        channel_idx
                    ]
                    if "channel_names" in metadata
...
def export_to_file(
    ...
    if format == "json":
        ...
        try:
            ...
            print("Warning: h5py not installed. Saving as NPZ instead.")
            export_to_file(dda_output, filename, format="npz", include_metadata=include_metadata)
